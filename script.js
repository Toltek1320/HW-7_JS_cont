...
// Обработчик события для кнопок "Add to Cart", собираем элемент и передаем его в функцию addToCart
...
// Массив id товаров в корзине и их кол-во
let myCart = []
...
function addToCart(item) {  
    ...
    // если товар есть в myCart, увеличиваем его кол-во, если нет добавляем в myCart и в корзину на сайте
    ...
    // обработчик события для кнопок "deleteFromCard"
...
function deleteItem(event) {
    ...
    event.remove();
    console.log(myCart);
    // если корзина пустая, то убираем текст. контент
    if (myCart.length === 0) {
        const cartItems = document.querySelector('.cartItems');
